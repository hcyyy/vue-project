    ...
    if (err) {
      ...
      async.waterfall([
        function (cb) {
          ...
          db.collection('shuju').find(param).count(function(err, num) {
            if (err) {
              cb(err);
            } else {
              cb(null, num);
        ...
        function (num, cb) {
          ...
          db.collection('shuju').find(param).skip(pageSize * pageNum - pageSize).limit(pageSize).toArray(function(err, data) {
            if (err) {
              cb(err);
            } else {
